...
function BentoCard({
  ...
  return (
    <motion.div
      ...
      className={cn(
        'glass gradient-border-hover rounded-2xl p-6 transition-all duration-300',
        className
      )}
    >
      {children}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 auto-rows-auto">
          ...
          <BentoCard className="md:col-span-2 md:row-span-2" delay={0}>
            <div className="flex flex-col h-full">
              <div className="mb-6 flex items-center gap-3">
                <div className="h-10 w-10 rounded-xl border border-indigo-500/20 bg-indigo-500/10 flex items-center justify-center">
          ...
          <BentoCard delay={0.15}>
            <div className="flex items-center gap-2 mb-4">
              ...
              <p className="text-xs font-medium text-emerald-400 uppercase tracking-widest">Available</p>
            </div>
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              {['Remote', 'Full-time', 'Freelance'].map(t => (
                <span key={t} className="rounded-md border border-emerald-500/20 bg-emerald-500/10 px-2.5 py-1 text-[11px] text-emerald-400">
                  {t}
                </span>
              ))}
            </div>
          ...
          <BentoCard delay={0.3} className="md:col-span-2">
            ...
            <div className="flex flex-wrap gap-2">
              {['Next.js 15', 'TypeScript', 'NestJS', 'PostgreSQL', 'Tailwind CSS', 'Framer Motion', 'Wagmi', 'Docker'].map(tech => (
                <span
                  key={tech}
                  className="rounded-lg border border-white/[0.07] bg-white/[0.03] px-3 py-1.5 text-xs font-medium text-zinc-300 transition-all hover:border-indigo-500/30 hover:bg-indigo-500/[0.08] hover:text-indigo-300"
                >
                  {tech}
                </span>
              ))}
